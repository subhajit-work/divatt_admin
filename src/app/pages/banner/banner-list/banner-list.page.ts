...
export class BannerListPage implements OnInit {
  ...
  statusChange(id) {
    ...
    // id:catid
    // }
    this.changeStatusSubscribe = this.http.put('adminMData/changeBannerStatus/' + id, '').subscribe(
      (res: any) => {
        this.commonUtils.presentToast('success', res.message);
      ...
      buttons: [
        {
          ...
          handler: () => {
            ...
            if (_identifire == 'single') {
              ...
              this.deleteDataSubscribe = this.http.put("adminMData/deleteBanner/"+_item.id, '').subscribe(
                (res: any) => {
