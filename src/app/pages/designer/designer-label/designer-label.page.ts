...
@Component({
  selector: 'app-designer-label',
  templateUrl: './designer-label.page.html',
  styleUrls: ['./designer-label.page.scss'],
})
export class DesignerLabelPage implements OnInit {
...
getLevelList()
{
  this.tableListSubscribe = this.http.get("adminMData/getDesignerCategory").subscribe(
    (res:any) => {
...
async onClickDeleteItem(_identifire, _item, _items, _index){
  ...
  handler: () => {
    ...
    if(_identifire == 'single'){
      ...
      this.deleteDataSubscribe = this.http.delete("adminMData/deleteDesignerLevels/"+ _item.Name).subscribe(
        (res:any) => {
    ...
    // ------------ single item delete end ------------
      ...
      this.deleteDataSubscribe = this.http.put('admin/muldelete', checkItemIdArray).subscribe(
      (res:any) => {
        if(res.status == 200){
          if(_items){
