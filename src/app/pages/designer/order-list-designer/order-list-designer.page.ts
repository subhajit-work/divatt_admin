...
@Component({
  selector: "app-order-list-designer",
  templateUrl: "./order-list-designer.page.html",
  styleUrls: ["./order-list-designer.page.scss"],
})
export class OrderListDesignerPage implements OnInit {
  ...
  commonFunction() {
    ...
    this.checkedList = [];
    this.url = environment.apiUrl+"/"+"userOrder/getOrderByInvoiceId";
    /*Check permission status start*/
    this.authService.globalparamsData.subscribe((res) => {
      console.log("res>>9", res);
      if (res.authority == "DESIGNER") {
        this.listing_url = "userOrder/list/"+res.uid;
        this.onRefresh();
  ...
  statusChange(catid) {
    ...
    this.changeStatusSubscribe = this.http.put("category/status", id).subscribe(
      (res: any) => {
        this.commonUtils.presentToast("success", res.message);
