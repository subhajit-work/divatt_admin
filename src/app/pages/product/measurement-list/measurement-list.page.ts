...
sortingButtonName: ""
  ...
  column_name: "Measurement Key",
  display_name: "Specification Value",
  sortingButtonName: ""
  }
];
isListLoading = false;
listing_url = 'productMeasurement/getAllMeasurementData';
tableData
pageNo;
sortColumnName = '';
sortOrderName = '';
...
if(res.authority == 'ADMIN'){
  // this.listing_url = 'specification/getAllSpecification';
  //       this.onRefresh();
...
if(data){
  ...
  for(let item of data) {
    console.log('item.modDetails.url',item.modDetails.url);
    
    if(item.modDetails.url == pageUrl[1]){
      if(item.modPrivs.list == true){
        console.log('-----Permission Granted-----',item.modDetails.url,pageUrl[1]);
        this.pagePermission = item;
        console.log('this.pagePermission', this.pagePermission);
        this.listing_url = 'productMeasurement/getAllMeasurementData';
        this.onRefresh();
          // delete api
        this.deleteApi = 'specification/delete';
        break;
      }else {
...
{
headers = "Delete"
messages = "Are you sure want to delete this Specifiction?"; 
}
...
statusChange(measurement)
{  
  console.log("measurement",measurement);
  
this.changeStatusSubscribe =this.http.put('productMeasurement/updateStatus/'+measurement.metaKey+'/'+measurement.id, '').subscribe(
  (res:any) => {
    this.commonUtils.presentToast('success', res.message);
...
handler: () => {
  ...
  if(_identifire == 'single'){
    ...
    this.deleteDataSubscribe = this.http.put("productMeasurement/deleteMeasurement/"+_item.metaKey+'/'+_item.id,'').subscribe(
      (res:any) => {
